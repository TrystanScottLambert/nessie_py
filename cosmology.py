...
class FlatCosmology:
    ...
    def comoving_distance(self, redshift: np.ndarray[float]) -> np.ndarray[float]:
        ...
        """
        return comoving_distances_at_z(
            redshift,
            self.omega_m,
            self.omega_k,
            self.omega_lambda,
    ...
    def z_at_comoving_distances(self, co_dist):
        """
        The inverse comoving distance function. Determines the redshift for the array of comoving
        distances.
        """
        return z_at_comoving_distances(
            co_dist, self.omega_m, self.omega_k, self.omega_lambda, self.hubble_constant
        )
    ...
    def h0_grow(self, redshifts: np.ndarray[float]):
        """
